...
--------------------------
Vibrato modulation plugin with tempo-synced LFO and stereo support.

Features:
- Tempo-synced divisions (1, 1/2, 1/4, 1/8, 1/16, 1/32)
- Stereo mode (L, R, LR)
- DAC calibration integration
- BMLPF stereo offset support
- Widget-driven depth control
"""
...
import showlog
from plugins.plugin_helper import division_label_from_index, rate_hz_from_division_label
from utils.rotating_state import RotatingState

# Plugin metadata
class VibratoPlugin(Plugin):
    """Vibrato Maker - Tempo-synced modulation plugin."""
    ...
    def on_load(self, app):
        ...
        try:
            ...
            showlog.error(f"[VibratoPlugin] Failed to register page: {e}")
