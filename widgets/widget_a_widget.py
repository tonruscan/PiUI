# widgets/widget_a_widget.py
import pygame
import showlog


class WidgetA:
    """Rectangular widget that visualises two values as progress bars."""
    ...
    # ------------------------------------------------------------------
    # Wiring helpers used by the host module
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Dirty rect bookkeeping (required by host)
    # ------------------------------------------------------------------
    def mark_dirty(self):
        self._dirty = True
    ...
    def handle_event(self, event):
        """Widgets without interactive controls simply ignore events."""
        return False
    ...
    def draw(self, surface, device_name=None, offset_y=0):
        ...
        panel_color = self.theme.get("plugin_background_color", (20, 20, 25))
        accent = self.theme.get("mini_dial_fill", (120, 180, 255))
        outline = self.theme.get("mini_dial_outline", (60, 90, 130))
        text_color = self.theme.get("dial_text_color", (230, 230, 230))
        ...
        for idx, key in enumerate(("dial1", "dial2")):
            ...
            label = self._font.render(f"A{idx + 1}: {value:03d}", True, text_color)
        ...
        self.clear_dirty()
        showlog.verbose(f"[WidgetA] Drawn at {panel_rect}")
        return panel_rect
